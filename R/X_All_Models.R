...
# ─────────────────────────────────────────────────────────────────────────────
# 1A) Pull out the estimated Matern parameters from spamm_SRF
# 1A) Get the BLUPs (one per unique lat/lng)
rL     <- ranef(spamm_SRF)
mat_re <- rL[["Matern(1 | lat + lng)"]]
uniq_coords <- unique(data[c("lat","lng")])
#stopifnot(nrow(uniq_coords)==length(mat_re))

# 1B) pairwise distances and theoretical correlation
dd <- dist(uniq_coords[,c("lng","lat")])
pars <- summary(spamm_SRF)$corrPars
mm <- spaMM::MaternCorr(
  as.numeric(dd),
  nu  = pars["1.nu"],
  rho = pars["1.rho"]
...
plot(
  as.numeric(dd), as.numeric(mm),
  pch=16, cex=0.4,
  xlab="Distance (deg)", 
  ylab="Matern correlation",
  main="Theoretical Matern correlation"
)

# ─────────────────────────────────────────────────────────────────────────────
# 2A) Install & load DHARMa if needed
if (!requireNamespace("DHARMa", quietly=TRUE)) install.packages("DHARMa")
library(DHARMa)

# 2B) Simulate and plot
sims <- simulateResiduals(fittedModel = spamm_SRF, n = 250)
plot(sims)

# ─────────────────────────────────────────────────────────────────────────────
# 3A) Choose a sequence of cum_gdd values
# build a toy grid varying just one covariate (cum_gdd)
newdat <- expand.grid(
  lat      = mean(data$lat),
  lng      = mean(data$lng),
  ...
  gdd      = mean(data$gdd),
  cum_gdd  = seq(min(data$cum_gdd), max(data$cum_gdd), length=100),
  croptype = factor("potato", levels=levels(data$croptype)),
  wei_intensity            = mean(data$wei_intensity),
  wei_prop                 = mean(data$wei_prop),
  summer_avg_temp          = mean(data$summer_avg_temp),
  summer_avg_percip        = mean(data$summer_avg_percip),
  summer_heavy_rainfall_days = mean(data$summer_heavy_rainfall_days),
  summer_hottest_temp      = mean(data$summer_hottest_temp),
  winter_coldest_temp      = mean(data$winter_coldest_temp),
  winter_extreme_cold_days = mean(data$winter_extreme_cold_days),
  winter_warm_day_count    = mean(data$winter_warm_day_count)
)

# drop all random effects: level = 0
newdat$fit <- predict(spamm_SRF,
                      newdata = newdat,
                      type    = "response",
                      level   = 0)
...
# plot
library(ggplot2)
ggplot(newdat, aes(x=cum_gdd, y=fit)) +
  geom_line() +
  labs(
    title = "Effect of cum_gdd (fixed only)",
    x     = "Cumulative GDD",
    y     = "Predicted beetles"
  ) +
  theme_minimal()
...
# ─────────────────────────────────────────────────────────────────────────────
# 4A) Build a regular grid over your region
grid <- expand.grid(
  lng = seq(min(data$lng), max(data$lng), length.out = 150),
  lat = seq(min(data$lat), max(data$lat), length.out = 150)
)

# 4B) Fill in each covariate at its dataset mean or modal level
grid$year     <- factor(levels(data$year)[1], levels = levels(data$year))
grid$farm     <- factor(levels(data$farm)[1], levels = levels(data$farm))
grid$gdd      <- mean(data$gdd)
grid$cum_gdd  <- mean(data$cum_gdd)
grid$croptype <- factor("potato", levels = levels(data$croptype))
grid$wei_intensity            <- mean(data$wei_intensity)
grid$wei_prop                 <- mean(data$wei_prop)
grid$summer_avg_temp          <- mean(data$summer_avg_temp)
grid$summer_avg_percip        <- mean(data$summer_avg_percip)
grid$summer_heavy_rainfall_days = mean(data$summer_heavy_rainfall_days)
grid$summer_hottest_temp      <- mean(data$summer_hottest_temp)
grid$winter_coldest_temp      <- mean(data$winter_coldest_temp)
grid$winter_extreme_cold_days <- mean(data$winter_extreme_cold_days)
grid$winter_warm_day_count    <- mean(data$winter_warm_day_count)

# 4C) Predict *including* the spatial + farm + year effects
grid$pred <- predict(spamm_SRF, newdata = grid, type = "response")

# 4D) Plot as a raster
library(ggplot2); library(viridis)
ggplot(grid, aes(x = lng, y = lat, fill = pred)) +
  geom_raster() +
  scale_fill_viridis_c(option="D") +
  coord_quickmap() +
  labs(
    title = "Predicted CPB abundance (full model)",
    x     = "Longitude", 
    y     = "Latitude", 
    fill  = "Beetles"
  ) +
